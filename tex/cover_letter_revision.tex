...
We are pleased to resubmit, as a revised manuscript, our paper \emph{``Foundation Models Match Supervised Forecasters on the Japan Electric Power Exchange: A Probabilistic 48-Step Benchmark Across Day-Ahead and Imbalance Markets''} for consideration as a research article in \emph{Applied Energy}. The retitle reflects the substantial change in the manuscript's headline finding under the expanded model set (see below). A detailed point-by-point response to each reviewer comment is attached as the document \texttt{RESPONSE\_TO\_REVIEWERS.pdf}.
