"""Run PatchTST under the Lago rolling-window protocol on 4 (area, market) pairs.

Same 365-day-window / weekly-recalibration / 305-day-test protocol as the
existing N-HiTS and LEAR runs. Univariate input: the 336-step raw price
lookback. Output: 48-step point forecast.

Saves results/patchtst_<area>_<market>_preds.csv and patchtst_metrics.json.
"""
...
from models_patchtst import PatchTST, train_patchtst
...
def make_fit_predict():
    def fit_predict(X_tr: pd.DataFrame, Y_tr: pd.DataFrame, X_te: pd.DataFrame) -> np.ndarray:
        ...
        n_val = max(48, int(0.1 * len(Xtr_arr)))
        model = PatchTST(lookback=LOOKBACK, horizon=HORIZON)
        model, _ = train_patchtst(model, Xtr_arr[:-n_val], Ytr_arr[:-n_val],
                                    Xtr_arr[-n_val:], Ytr_arr[-n_val:],
                                    epochs=15, batch=256, lr=5e-4)
        model.eval()
        with torch.no_grad():
            yhat = model(torch.tensor(Xte_arr)).cpu().numpy()
        return yhat
    return fit_predict


def run_one(area: str, market: str):
    print(f"\n=========== {area} {market} PatchTST ===========")
    ...
    pd.DataFrame(preds, index=test_idx,
                 ...
                 ).to_csv(os.path.join(OUT, f"patchtst_{area}_{market}_preds.csv"))
...
def main():
    out_path = os.path.join(OUT, "patchtst_metrics.json")
    out = json.load(open(out_path)) if os.path.exists(out_path) else {}
    for area in ["TK", "KS"]:
        for market in ["DA", "IM"]:
            key = f"{area}_{market}"
            pred_file = os.path.join(OUT, f"patchtst_{area}_{market}_preds.csv")
