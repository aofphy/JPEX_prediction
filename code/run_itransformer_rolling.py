"""Run iTransformer under the Lago rolling-window protocol on 4 (area, market) pairs.

Same protocol as run_patchtst_rolling.py; differs only in model.
"""
...
from models_itransformer import ITransformer, train_itransformer
...
def make_fit_predict():
    def fit_predict(X_tr: pd.DataFrame, Y_tr: pd.DataFrame, X_te: pd.DataFrame) -> np.ndarray:
        ...
        model = ITransformer(lookback=LOOKBACK, horizon=HORIZON)
        model, _ = train_itransformer(model, Xtr_arr[:-n_val], Ytr_arr[:-n_val],
...
def run_one(area: str, market: str):
    ...
    pd.DataFrame(preds, index=test_idx,
                 ...
                 ).to_csv(os.path.join(OUT, f"itransformer_{area}_{market}_preds.csv"))
...
def main():
    out_path = os.path.join(OUT, "itransformer_metrics.json")
    out = json.load(open(out_path)) if os.path.exists(out_path) else {}
    for area in ["TK", "KS"]:
        for market in ["DA", "IM"]:
            key = f"{area}_{market}"
            pred_file = os.path.join(OUT, f"itransformer_{area}_{market}_preds.csv")
